...
# Fargate가 ECR pull, CloudWatch Logs 작성, Secrets 읽기에 사용
...
# 컨테이너 내부 애플리케이션이 AWS 서비스에 접근할 때 사용
...
# MediaMTX needs CloudWatch logs only (no S3/SQS access)
...
# Terraform CI/CD에서 모든 리소스를 관리할 수 있도록 AdministratorAccess 부여
